...
class BundleFileReadOnlyViewSet(viewsets.ViewSet):
    ...
    def get_bundle_version(self, bundle_slug, version_num=None):
        """ Get the bundle version. """

        filter_kwargs = {
            'bundle__slug': bundle_slug
        }
        if version_num:
    ...
    def get_bundle_version_or_404(self, bundle_slug, version_num=None):
        bundle_version = self.get_bundle_version(bundle_slug, version_num)
        if bundle_version:
            return bundle_version
        raise http.Http404

    def list(self, _request, bundle_slug, version_num=None):
        """ Retrieve all files in a bundle. """
        snapshot = self.get_bundle_version_or_404(bundle_slug, version_num).snapshot()
    ...
    def retrieve(self, _request, bundle_slug, path, version_num=None):
...
class BundleFileViewSet(BundleFileReadOnlyViewSet):
    ...
    def create(self, request, bundle_slug, version_num=None):
        ...
        if serializer.is_valid():
            ...
            if bundle_version:
                ...
                snapshot = BundleSnapshot.create(slug, {})
    ...
    def destroy(self, _request, bundle_slug, version_num=None, path=None):
