...
If you need such overrides, create a file :file:`blockstore/settings/private.py`. This file's values are
read by :file:`blockstore/settings/local.py`, but ignored by Git.
...
Configuring Blockstore to work with OIDC requires registering a new client with the authentication
...
A new OAuth 2.0 client can be created at ``http://127.0.0.1:8000/admin/oauth2/client/``.
    ...
    3. Specify the name of this service, ``Blockstore``, as the client name.
...
:file:`blockstore/settings/local.py`). The table below describes the relevant settings.
